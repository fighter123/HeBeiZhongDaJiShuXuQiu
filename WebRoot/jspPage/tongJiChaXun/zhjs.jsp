...
<html>
  ...
  <script>
	...
	function addDiv()
	{
		...
		var checkValue=$("select[name='firstindex01']:last").val();    //获取最后一个name=firstindex01的Select选择的Value 	
		alert(checkValue);
		var cnode=node.cloneNode(true);
		$(cnode).children().eq(2).attr("name","firstindex01"); //将复制后元素的第三个下拉框name属性设置为firstindex01
		var c=$(cnode).children().eq(2).children("option[value="+checkValue+"]").remove();   //删除Select中Value=checkValue的Option
		//alert($(cnode).children().eq(2).children().size());
		//alert(c);
		cnode.style.display="";
		if(num<4)
		{  
	...
	function addDiv1()
	{
		...
		var checkValue=$("select[name='firstindex11']:last").val();    //获取最后一个name=firstindex01的Select选择的Value 	
		//alert(checkValue);
		var cnode=node.cloneNode(true);
		$(cnode).children().eq(2).attr("name","firstindex11"); //将复制后元素的第三个下拉框name属性设置为firstindex01
		var c=$(cnode).children().eq(2).children("option[value="+checkValue+"]").remove();   //删除Select中Value=checkValue的Option
		cnode.style.display="";
		if(num1<8)
		{
  ...
  <body>
  			...
  			<h4>输入检索条件:</h4>
				<div id="div0">
					...
					<div>	
						...
						<select id="select1" style="width:120px" name="firstindex01">
  	...
  	<div id="div1" style="display:none">
					 	<div Style="height:5px"></div>
					 	<select style="width:62px" name="selecttype01">
					 		<option value="and">并含</option>
					 		<option value="or">或含</option>
					 		<option value="not">不含</option>
					 	</select>
